...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['initiate_mpesa'])) {
    try {
        ...
        if (!$mpesa_response || (isset($mpesa_response['ResponseCode']) && $mpesa_response['ResponseCode'] !== '0')) {
            $errorMsg = isset($mpesa_response['errorMessage']) ? $mpesa_response['errorMessage'] : (isset($mpesa_response['errorMessage']) ? $mpesa_response['errorMessage'] : 'Failed to initiate M-Pesa payment.');
            throw new Exception('M-Pesa initiation failed: ' . $errorMsg);
        }
                        ...
                        <div class="payment-methods mt-4">
                            ...
                            <div class="form-check">
                                ...
                                <div id="mpesa-details" class="mt-2 p-3 bg-light rounded">
                                    ...
                                    <button type="button" id="initiateMpesaBtn" class="btn btn-primary mt-2">
                                        Initiate M-Pesa Payment
                                    </button>
                                    <div id="mpesa-initiate-message"></div>
            ...
            <div class="card shadow-sm">
                ...
                <div class="card-body">
                    ...
                    <div class="d-flex justify-content-between mb-2">
                        ...
                        <strong id="delivery-fee-value">Ksh <?= number_format($order_totals['delivery_fee'], 2) ?></strong>
                    </div>
                    <hr>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const mpesaPaymentInitiatedInput = document.getElementById('mpesa_payment_initiated');
        const initiateMpesaBtn = document.getElementById('initiateMpesaBtn');
        const mpesaPhoneInput = document.getElementById('mpesa_phone');
        const countyInput = document.getElementById('county');
        const mpesaInitiateMessage = document.getElementById('mpesa-initiate-message');

        initiateMpesaBtn.addEventListener('click', function(e) {
            e.preventDefault();
            initiateMpesaBtn.disabled = true;
            mpesaInitiateMessage.innerHTML = '<span class="text-info">Initiating payment...</span>';
            ...
            fetch('mpesa_initiate.php', {
                ...
                .then(data => {
                    if (data.success) {
                        mpesaInitiated = true;
                        if (mpesaPaymentInitiatedInput) mpesaPaymentInitiatedInput.value = '1';
                        mpesaInitiateMessage.innerHTML = '<div class="alert alert-success mt-3">' + data.message + '</div>';
                    } else {
                        mpesaInitiated = false;
                        if (mpesaPaymentInitiatedInput) mpesaPaymentInitiatedInput.value = '0';
                        mpesaInitiateMessage.innerHTML = '<div class="alert alert-danger mt-3">' + data.message + '</div>';
                    }
                ...
                .catch(err => {
                    mpesaInitiated = false;
                    if (mpesaPaymentInitiatedInput) mpesaPaymentInitiatedInput.value = '0';
                    mpesaInitiateMessage.innerHTML = '<div class="alert alert-danger mt-3">Failed to initiate payment. Please try again.</div>';
                    initiateMpesaBtn.disabled = false;
                });
        });
        ...
        if (checkoutForm) {
            checkoutForm.addEventListener('submit', function(e) {
                ...
                if (paymentMethod === 'mpesa') {
                    if (!mpesaInitiated && mpesaPaymentInitiatedInput.value !== '1') {
                        e.preventDefault();
        ...
        document.getElementById('county').addEventListener('change', function() {
            const deliveryFeeElement = document.getElementById('delivery-fee-value');
